...
# Absolute path to this skill's references directory (used in error messages)
_SKILL_REF_DIR = Path(__file__).parent.resolve()
...
def render(
    ...
    try:
        ...
        print(f"Run: cd {_SKILL_REF_DIR} && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(f"Run: cd {_SKILL_REF_DIR} && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
