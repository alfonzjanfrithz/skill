...
* When everything done MOVE the .excalidraw file that we have created on to the user working directory for example `docs/excalidraw/<output.excalidraw | output.png>`
---
...
RUN This to make sure there is no bug on your diagram. Refer to `references/validation.md`. There are collection of checklist to guide you writing properly. There are known bugs that you may want to check too.
...
```bash
cd <skill-path>/excalidraw/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```

*(Replace `<skill-path>` with your agent's skills directory. Common paths: `.claude/skills`, `.cursor/skills`, `.opencode/skills`)*
...
### First-Time Setup
If the render script hasn't been set up yet, run these commands from the skill's `references` directory:
```bash
